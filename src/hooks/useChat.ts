...
export function useChat(options: UseChatOptions): UseChatReturn {
  ...
  persistMessagesRef.current = (msgs?: ChatMessage[]) => {
    ...
    try {
      ...
      // Non-critical — the backend's own addMessage is a fallback.
    }
  };
  ...
  useEffect(() => {
    ...
    const unsubToolCall = api.on.chatStreamToolCall((data: { sessionId: string; toolCall: Record<string, unknown> }) => {
      ...
      // tool call — each has the same ID. Previously we appended all 3 →
      // 3 duplicate entries → the extra ones never got a result → stuck
      // spinners. Now we update the existing entry if the ID matches.
    ...
    const unsubToolResult = api.on.chatStreamToolResult((data: { sessionId: string; toolResult: Record<string, unknown> }) => {
      ...
      // Phase 1.1 + 2.2: Detect denied AND deactivated tool results.
      const isDenied = toolResult?.denied === true;
      const isDeactivated = toolResult?.deactivated === true;
      ...
      const applyResult = (list: ToolCall[]): ToolCall[] =>
        list.map(tc =>
          tc.id === resultId
            ? { ...tc, result: resultValue, status: newStatus as any, _pendingPermission: undefined }
            : tc
        );
    ...
    const unsubError = api.on.chatStreamError((data: { sessionId: string; error: string }) => {
      ...
      setMessages(prev => {
        ...
        const lastMsg = updated[updated.length - 1];
        if (lastMsg && lastMsg.role === 'assistant' && lastMsg.isStreaming) {
          // Phase 2.5: Mark any pending tool calls as 'failed' on error.
