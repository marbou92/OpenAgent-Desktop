 ...
 * - macOS: Lima VM sandbox
 * - Windows 7: Basic sandbox (restricted process)
 * - Linux: Docker container sandbox
 *
 * Features:
 * - Resource limits (CPU, memory, disk)
 * - Network isolation options
 * - Filesystem access controls (allowlist/denylist)
...
`;
    ...
    if (config.networkIsolation) {
      // WSL2 doesn't natively support network isolation,
      // but we can restrict via firewall rules
      try {
        this.execSync(
  ...
  async execute(command: string, options?: ExecuteOptions): Promise<ExecuteResult> {
    ...
    let stdinArg = "";
    if (options?.stdin) {
      // Write stdin to a temp file and pipe it
  ...
  private async setupFilesystemAccess(config: SandboxConfig): Promise<void> {
    // Mount allowed paths into WSL
    if (config.allowedPaths && config.allowedPaths.length > 0) {
      for (const allowedPath of config.allowedPaths) {
        // WSL2 auto-mounts Windows drives under /mnt/
...
class LimaSandbox implements SandboxInterface {
  ...
  async putFile(filePath: string, content: Buffer): Promise<void> {
    ...
    try {
      // Use limactl copy to put file into the VM
      this.execQuiet(
...
class DockerSandbox implements SandboxInterface {
  ...
  async start(config: SandboxConfig): Promise<void> {
    ...
    // Check if image exists, if not pull Ubuntu
...
class BasicSandbox implements SandboxInterface {
  ...
  async start(config: SandboxConfig): Promise<void> {
    ...
    // In basic mode, we set up a restricted working directory
    // We cannot provide true isolation without VM/container support
    console.warn(
  ...
  async getFile(filePath: string): Promise<Buffer> {
    // In basic mode, files are on the host filesystem
  ...
  private resolveSandboxPath(filePath: string): string | null {
    // In basic mode, resolve paths relative to workDir
  ...
  private startHealthMonitoring(): void {
    ...
    this.healthCheckTimer = setInterval(() => {
      ...
      // Basic health check - just verify workspace exists
      if (fs.existsSync(this.workDir)) {
        // Update disk usage
        try {
...
export class SandboxManager extends EventEmitter {
  ...
  detectSandboxType(): SandboxType {
    ...
    if (platform === "win32") {
      ...
      try {
        ...
        // Fall back to basic sandbox if Docker not available
        console.warn(
          "[SandboxManager] Docker not available, falling back to basic sandbox"
        );
        return "basic";
      }
    }
  ...
  async start(config?: SandboxConfig): Promise<void> {
    ...
    if (!this.sandbox) {
      this.sandboxType = this.detectSandboxType();
  ...
  async getFile(filePath: string): Promise<Buffer> {
    ...
    return this.sandbox.getFile(filePath);
  }
  ...
  async putFile(filePath: string, content: Buffer): Promise<void> {
    ...
    return this.sandbox.putFile(filePath, content);
  }
